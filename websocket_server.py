            ...
            if self.handshaken == False:  # 如果没有进行握手
                ...
                if self.length_buffer - g_header_length < g_code_length:
                    ...
                    if PRINT_FLAG:
                        ...
                        sendMessage("server receive client message: INFO Line 204: Recv信息 %s,长度为 %d:" % (self.buffer_utf8, len(self.buffer_utf8)))
                    ...
                    if recv_message == "quit":
                        ...
                        sendMessage("client send to server： %s %s say: %s" % (nowTime, self.remote, self.name + " Logout"))
                        sendMessage("server sent to client： %s %s say: %s" % (nowTime, self.remote, self.name + " Logout"))
                        deleteconnection(str(self.index))
                        self.conn.close()
                        break
                    else:
                        nowTime = time.strftime('%H:%M:%S', time.localtime(time.time()))
                        sendMessage("server sent to client: %s %s say: %s" % (nowTime, self.remote, recv_message))
                    g_code_length = 0
                    self.length_buffer = 0
                    self.buffer_utf8 = b""
